...
exports.getAllProjects = async (req) => {
  try {
    const { search, technologies, level, sortBy = "latest" } = req.query;
    ...
    if (technologies) {
      ...
      const techDocs = await technologyModel.find({
        $or: [{ slug: { $in: techArray } }, { name: { $in: techArray } }]
      });
...
exports.getAllMyProjects = async (userId) => {
  try {
    const projects = await projectRepository.find({ authorId: userId });
    return projects;
  } catch (error) {
    throw error;
  }
...
exports.getProjectBySlug = async (slug) => {
  try {
    const project = await ProjectModel.findOne({ slug, isPublished: true })
      .populate("authorId", "displayName photo email")
      .populate("learningPackId", "title description videoUrls documents")
      .select(
        "title slug description thumbnailUrl demoVideoUrl level technologies price discountPrice createdAt updatedAt authorId isPublished isBundleItem learningPackId"
      );

    if (!project) {
      throw new AppError("Không tìm thấy hoặc project không được công khai");
    }
    ...
    const similarProjects = await ProjectModel.find({
      ...
      .select(
        "title slug description thumbnailUrl level technologies price discountPrice"
      )
    ...
    const technologyIds = await Promise.all(
      technologies.map(async (techSlugOrName) => {
        const tech = await technologyModel.findOne({
          $or: [{ slug: techSlugOrName }, { name: techSlugOrName }]
        });
